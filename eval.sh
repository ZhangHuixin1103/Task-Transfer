...
# python eval_lumina.py --use_qwen_for_prompt --speculative_jacobi --quant
# python eval_lumina.py --fixed_prompt "This is a visual in-context learning task. The first two images are an input and output of Task A. The third image is the input for Task B. The goal is to perform Task B on the third image and generate output image, learning from Task A." --speculative_jacobi --quant
